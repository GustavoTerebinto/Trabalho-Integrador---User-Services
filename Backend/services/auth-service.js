		...
		new LocalStrategy(
			...
			async (username, password, done) => {
				try {
					...
					const user = await model.UsuarioAcesso.findOne({
...
const configureJwtStrategy = () => {
	passport.use(
		new JwtStrategy(
			...
			async (payload, done) => {
				try {
					const user = await model.UsuarioAcesso.findByPk(payload.username);
...
const criarNovoUsuario = async (userData) => {
	...
	const userName = userEmail; // Usa o email como nome padrão se não fornecido
	const salt = bcrypt.genSaltSync(saltRounds);
	const hashedPasswd = bcrypt.hashSync(passwd, salt);

	await model.UsuarioAcesso.create({
		email: userEmail,
		senha: hashedPasswd,
	});
